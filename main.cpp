...
bool isValid1(const string& email){
  
    // Regular expression
    const regex pattern(
        "(\\w+)(\\.|_)?(\\w*)@(\\w+)(\\.(\\w+))+");
  
    // Matching the string pattern with the regular expression
...
bool isValid2(string s)
{
  ...
  //Match between given number and regular expression
